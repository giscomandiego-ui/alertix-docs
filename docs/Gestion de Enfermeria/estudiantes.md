---
title: Estudiantes
deprecated: false
hidden: false
metadata:
  robots: index
---
Alertix centraliza la información de los estudiantes para facilitar su identificación y el acceso a los datos necesarios durante la atención.

### Perfil del estudiante

Cada estudiante cuenta con un perfil donde se puede consultar la información registrada, incluyendo sus datos personales y médicos.

### Información médica

El perfil puede incluir información relevante como:

- Alergias.
- Condiciones médicas.
- Medicamentos.
- Información proporcionada por los padres de familia.
- Documentos médicos.

### Código QR

Cada estudiante puede contar con un código QR que permite identificar rápidamente su perfil dentro de Alertix.

### Vinculación familiar

Los estudiantes pueden estar vinculados a sus padres o tutores para que estos puedan consultar la información correspondiente desde sus propias cuentas.
